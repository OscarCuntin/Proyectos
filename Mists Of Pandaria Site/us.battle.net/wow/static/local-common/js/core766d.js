...
var Tickets = {
	...
	localizeDatetime: function(timestamp) {
		...
		if (locale === 'en-us' || locale === 'es-mx' || locale === 'zh-cn' || locale === 'zh-tw') {
			datetime = datetime.replace('/0', 'http://us.battle.net/');
...
var CharSelect = {
	...
	pin: function(index, link) {
		...
		$.ajax({
			...
			success: function(content) {
				...
				if (location.pathname.indexOf('http://us.battle.net/character/') != -1) {
					if (location.pathname.indexOf('http://us.battle.net/vault/') != -1)
						location.reload(true);
					else
	...
	afterPageUpdate: function() {
		...
		if (location.href.indexOf('http://us.battle.net/character/') != -1) {
			redirectTo = $('#user-plate a.character-name').attr('href');

		} else if (location.href.indexOf('http://us.battle.net/guild/') != -1) {
	...
	redirectTo: function(url) {
		...
		if (url.indexOf('http://us.battle.net/vault/') != -1) {
			location.reload();
			return;
		}
