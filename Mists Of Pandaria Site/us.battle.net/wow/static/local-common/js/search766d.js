...
var Search = {
	...
	initialize: function(url, searchField) {
		...
		if (Core.project == 'wow') {
			...
			resultTypes = ['article', 'static', 'other', 'kb'];
			}
...
var TypeAhead = Class.extend({
	...
	listen: function(e) {
		...
		if (value.length >= this.config.minLength) {
			...
			if (typeof this.ghost !== 'undefined' && this.ghost.val().toLowerCase().indexOf(this.term.toLowerCase()) < 0) {
				this._ghostText();
			}
			this.throttle();
	...
	_populate: function() {
		...
		if (this.ghost) {
			...
			if (counter) {
				text = this.cache[0].title;

				if (text.toLowerCase().indexOf(this.term.toLowerCase()) === 0) {
					text = text.replace(new RegExp(this.term, 'i'), this.term);
				}
			}

			if (text.indexOf(this.node.val()) >= 0) {
