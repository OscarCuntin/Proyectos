...
var Wow = {
	...
	bindCharacterTooltips: function() {
		Tooltip.bind('a[href*="/character/"]', false, function() {
			if (this.href == 'javascript:;' || this.href.indexOf('#') == 0 || this.rel == 'np' || this.href.indexOf('http://us.battle.net/vault/') != -1)
				return;

			var href = $(this).attr('href').replace(Core.baseUrl +'/character/', "").split('http://us.battle.net/');
	...
	browseArmory: function(node, url) {
		...
		Menu.show(node, 'http://us.battle.net/', { set: 'browse' });
	},
